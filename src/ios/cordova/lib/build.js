...
module.exports.run = function (argv) {
    ...
    return check_reqs.run().then(function () {
        ...
        if(isWorkspaceExist(projectPath, projectName)){
          xcodebuildArgs = getXcodeArgs(projectName, projectPath, configuration, args.device);
        } else {
          xcodebuildArgs = getXcodeWorkspaceArgs(projectName, projectPath, configuration, args.device);
        }
...
/**
 * Returns array of arguments for xcodebuild when doing Workspace
 * @param  {String}  projectName   Name of xcode project
 * @param  {String}  projectPath   Path to project file. Will be used to set CWD for xcodebuild
 * @param  {String}  configuration Configuration name: debug|release
 * @param  {Boolean} isDevice      Flag that specify target for package (device/emulator)
 * @return {Array}                 Array of arguments that could be passed directly to spawn method
 */
function getXcodeWorkspaceArgs(projectName, projectPath, configuration, isDevice) {
    var xcodebuildArgs;
    if (isDevice) {
        xcodebuildArgs = [
            '-xcconfig', path.join(__dirname, '..', 'build-' + configuration.toLowerCase() + '.xcconfig'),
...
function isWorkspaceExist(projectPath, projectName){
  var wsPath = path.join(projectPath, projectName + '.xcworkspace');
  try {
        return fs.lstatSync(wsPath).isDirectory();
    }
    catch (err) {
        return false;
    }
